...
if role == "student":
    ...
    for i, t in enumerate(relevant):
        ...
        st.markdown(f"- 📌 **{t['text']}** ({t['category']}) — Due: {t['target_date']}<br/>Groups: {group_display}", unsafe_allow_html=True)
            if st.button("Import to My Goals", key=f"import_{uuid.uuid4()}"):
                new_goal = {
                    'text': t['text'],
                    'category': t['category'],
                ...
                if new_goal not in st.session_state.user_goals[username]:
                    st.session_state.user_goals[username].append(new_goal)
