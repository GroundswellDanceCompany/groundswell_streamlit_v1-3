...
# ---- TEMPLATE DISPLAY SECTION ----
    is_teacher = st.session_state.USER_DB[st.session_state.username]["role"] == "admin"
    user_classes = st.session_state.USER_DB[st.session_state.username].get("groups", [])
    available_templates = [
    t for t in st.session_state.templates if is_teacher or t["class"] in user_classes
    ]
    ...
    with col2:
        if st.button("Delete", key=f"delete_{t['id']}"):
            st.session_state.templates = [temp for temp in st.session_state.templates if temp["id"] != t["id"]]
