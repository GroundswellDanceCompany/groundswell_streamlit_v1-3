...
def show_teacher_dashboard(user):
    st.title(f"Welcome, {user} (Teacher)")

    st.subheader("Class Groups")
    db = st.session_state.USER_DB
    grouped = {}
    ...
    for u, data in db.items():
        if data["role"] == "student":
            for group in data.get("groups", []):
                grouped.setdefault(group, []).append(u)

    for group, students in grouped.items():
        with st.expander(group):
            st.markdown("**Students:** " + ", ".join(students))
            for student in students:
                goals = st.session_state.user_goals.get(student, [])
                st.markdown(f"*{student}'s goals:*")
                for goal in goals:
                    st.markdown(f"- {goal}")

    st.subheader("Create Template Goals")
    selected_class = st.selectbox("Select class", CLASS_GROUPS)
    template_goal = st.text_input("Template goal")
    if st.button("Save Template"):
        if "templates" not in st.session_state:
            st.session_state.templates = {}
        st.session_state.templates.setdefault(selected_class, []).append(template_goal)
...
def main():
    ...
    # ---- Logout Button ----
    st.sidebar.markdown("### User: " + str(st.session_state.get("current_user", "")))
