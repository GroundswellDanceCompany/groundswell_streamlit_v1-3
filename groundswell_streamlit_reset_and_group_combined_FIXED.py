...
if role == "student":
    ...
    relevant = [t for t in st.session_state.templates if set(t.get("groups", [])) & set(groups)]
    for i, t in enumerate(relevant):
        st.markdown(f"- **{t['text']}** ({t['category']}) — Due: {t['target_date']} — _Groups: {', '.join(t['groups'])}_")
        if st.button("Import to My Goals", key=f"import_{i}"):
